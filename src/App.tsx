...
import { AppsScriptConfigView } from './components/AppsScriptConfigView';
...
export default function App() {
  ...
  const [appsScripts, setAppsScripts] = useState<AppsScriptApp[]>([]);
  ...
  useEffect(() => {
    const engine = new KeepSyncEngine({
      ...
      onAppsScriptsUpdated: (updatedAppsScripts) => {
        setAppsScripts([...updatedAppsScripts]);
      },
  ...
  const handleRequestDeleteLabel = (label: string) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        ...
        if (typeof currentView === 'object' && 'label' in currentView && currentView.label === label) {
  ...
  const handleSaveAppsScripts = (updated: AppsScriptApp[]) => {
    const capped = updated.slice(0, 5);
    setAppsScripts(capped);
    syncEngineRef.current?.updateAppsScripts(capped);
  };
  ...
  const filteredNotes = useMemo(() => {
    ...
    } else if (typeof currentView === 'object' && 'label' in currentView) {
  ...
  const isLabelView = typeof currentView === 'object' && 'label' in currentView;
  const isNotesOrLabelView = currentView === 'notes' || isLabelView;

  const pinnedNotes = useMemo(() => {
    if (!isNotesOrLabelView) return [];
    return filteredNotes.filter((n) => n.isPinned);
  }, [filteredNotes, isNotesOrLabelView]);

  const unpinnedNotes = useMemo(() => {
    if (!isNotesOrLabelView) return filteredNotes;
    if (pinnedNotes.length === 0) return filteredNotes;
    return filteredNotes.filter((n) => !n.isPinned);
  }, [filteredNotes, pinnedNotes, isNotesOrLabelView]);
  ...
  const activeLabelString = typeof currentView === 'object' && 'label' in currentView ? currentView.label : undefined;
  const isAppsScriptView = typeof currentView === 'object' && 'appScriptId' in currentView;
    ...
    <div id="app-root" className="min-h-screen bg-[#fafaf9] dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 flex flex-col font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <Sidebar
          ...
          labels={labels}
          notes={notes}
          appsScripts={appsScripts}
          isOpen={isSidebarOpen}
          onOpenLabelManager={() => setLabelManagerOpen(true)}
          onOpenSecurity={() => setSecurityModalOpen(true)}
          onOpenAppsScriptConfig={() => setCurrentView('apps-script-config')}
        />
        ...
        >
          {currentView === 'apps-script-config' ? (
            <AppsScriptConfigView
              appsScripts={appsScripts}
              onSaveAppsScripts={handleSaveAppsScripts}
              onOpenApp={(appId) => setCurrentView({ appScriptId: appId })}
              onBackToNotes={() => setCurrentView('notes')}
            />
          ) : typeof currentView === 'object' && 'appScriptId' in currentView ? (
            (() => {
              const selectedApp = appsScripts.find((a) => a.id === currentView.appScriptId);
              if (!selectedApp) {
                return (
                  <div className="max-w-md mx-auto py-16 text-center space-y-4 px-4">
                    <p className="text-base font-semibold text-neutral-800 dark:text-neutral-200">
                      Apps Script Not Found
                    </p>
                    <p className="text-xs text-neutral-500">
                      This application URL may have been removed or changed in the configuration.
                    </p>
                    <button
                      onClick={() => setCurrentView('apps-script-config')}
              ...
              return (
                <AppsScriptViewer
                  app={selectedApp}
                  onOpenConfig={() => setCurrentView('apps-script-config')}
                />
              );
            })()
          ) : (
            <>
              ...
              {(currentView === 'notes' || (typeof currentView === 'object' && 'label' in currentView)) && !searchQuery && (
                <NoteEditor
                  labels={labels}
              ...
              {filteredNotes.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center text-neutral-400 space-y-3">
                  <div className="w-16 h-16 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-400 mb-1">
                    ...
                    {typeof currentView === 'object' && 'label' in currentView && <Tag className="w-8 h-8" />}
                  </div>
